...
test("autoroller options commands and atomic settings use the active roller",function()
  local f=fake(); local hud=Main.new(f,{layout={},roller={target_total=53,hard_stop=62,reroll_delay=.1,reroll_command="reroll",use_min_stats=true,min_stats={STR=5}}}); assert(hud:start())
  ...
  local ok,err=f.rollerSettingsCallback({target_total="60",hard_stop="62",reroll_delay="0.2",reroll_command="reroll",use_min_stats=true,min_stats={STR="6",MP="5"}}); assert(ok,err); eq(hud.roller.cfg.target_total,60); eq(hud.roller.cfg.min_stats.MP,5); eq(f.savedRollerSettings.target_total,60)
  hud:shutdown()
end)
