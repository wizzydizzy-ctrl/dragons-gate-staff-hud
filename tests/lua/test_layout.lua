...
test("hidden chat returns its entire height to the main display",function()
  for _,size in ipairs({{760,700},{1200,800},{1920,1080},{3840,2160}}) do
    local shown=Layout.compute(size[1],size[2],{visible=true})
    local hidden=Layout.compute(size[1],size[2],{visible=false})
    eq(shown.chat_visible,true); eq(hidden.chat_visible,false)
    eq(hidden.chat_height,0); eq(hidden.chat_output_height,0)
    eq(hidden.console_top,hidden.header_height); eq(hidden.top,hidden.header_height)
    eq(hidden.console_width,shown.console_width); eq(hidden.bottom,shown.bottom)
    eq(hidden.console_remainder,shown.console_remainder+shown.chat_height)
    local restored=Layout.compute(size[1],size[2],{visible=true})
    eq(restored.console_top,shown.console_top); eq(restored.chat_height,shown.chat_height)
  end
end)
test("hidden chat stays zero height through tiny and narrow resizes",function()
  for _,size in ipairs({{120,150},{320,260},{400,300},{760,150},{1200,200}}) do
    local shown=Layout.compute(size[1],size[2])
    local hidden=Layout.compute(size[1],size[2],{visible=false,target_height=300})
    eq(hidden.chat_height,0); eq(hidden.chat_functional_minimum,0)
    eq(hidden.console_top+hidden.bottom+hidden.console_remainder,size[2])
    eq(hidden.console_remainder>=shown.console_remainder,true)
  end
end)
test("old chat settings without visibility remain shown",function()
  eq(Layout.compute(1200,800).chat_visible,true)
  eq(Layout.compute(1200,800,{enabled=true}).chat_visible,true)
end)
...
test("side text presets resize HUD cards without changing the main display",function()
  local small=Layout.compute(1200,800,nil,nil,nil,{side_text_scale=.9})
  local normal=Layout.compute(1200,800,nil,nil,nil,{side_text_scale=1})
  local large=Layout.compute(1200,800,nil,nil,nil,{side_text_scale=1.1})
  eq(small.body_font<normal.body_font,true); eq(large.body_font>normal.body_font,true)
  eq(small.list_font<normal.list_font,true); eq(large.list_font>normal.list_font,true)
  eq(small.console_width,normal.console_width); eq(large.console_width,normal.console_width)
  eq(small.chat_wrap_columns,normal.chat_wrap_columns); eq(large.chat_wrap_columns,normal.chat_wrap_columns)
end)
